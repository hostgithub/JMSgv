...
public class GovInfoAdapter extends RecyclerView.Adapter<BaseViewHolder> {
    private static final int TYPE_FOOTER = 0;
    private static final int ITEM_NOIMAGE = 1;
    private static final int ITEM_HASIMAGE = 2;

    private ArrayList<Gongzuonianbao.ResultsBean> datas;
    private Context mContext;
    private int viewFooter;
    private View footerView;
    private OnItemClickListener mOnItemClickListener;

    public GovInfoAdapter(Context context, ArrayList<Gongzuonianbao.ResultsBean> datas) {
        this.datas = datas;
        this.mContext = context;
    }

    public void replaceAll(List<Gongzuonianbao.ResultsBean> elements) {
        if (datas.size() > 0) {
            datas.clear();
        }
    ...
    public void addAll(List<Gongzuonianbao.ResultsBean> elements) {
        datas.addAll(elements);
        notifyDataSetChanged();
    }
    @Override
    ...
    public void onBindViewHolder(BaseViewHolder holder, final int position) {
        if (!(viewFooter != 0 && position == getItemCount() - 1)) {
            ...
            final Gongzuonianbao.ResultsBean item = datas.get(position);
