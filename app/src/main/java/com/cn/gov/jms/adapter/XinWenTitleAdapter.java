...
/**
 * Created by wangjiawei on 2017/12/5.
 * 新闻中心栏目列表适配器
 */

public class XinWenTitleAdapter extends RecyclerView.Adapter<BaseViewHolder> {
    ...
    public XinWenTitleAdapter(Context context, List<String> datas) {
        this.datas = datas;
...
//    }
    ...
    public BaseViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == ITEM_NOIMAGE) {
            return new BaseViewHolder(LayoutInflater.from(mContext).inflate(R.layout.item_news_title, parent, false));
    ...
    public void onBindViewHolder(BaseViewHolder holder, final int position) {
        if (!(viewFooter != 0 && position == getItemCount() - 1)) {
            ...
            final String item = datas.get(position);
...
//            }
            ...
            holder.setText(R.id.tv_title, datas.get(position));
