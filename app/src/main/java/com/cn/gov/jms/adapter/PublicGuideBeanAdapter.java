...
public class PublicGuideBeanAdapter extends RecyclerView.Adapter<BaseViewHolder> {
    private static final int TYPE_FOOTER = 0;
    private static final int ITEM_NOIMAGE = 1;
    private static final int ITEM_HASIMAGE = 2;

    private List<PublicGuideBean.ResultsBean> datas;
    private Context mContext;
    private int viewFooter;
    private View footerView;
    private OnItemClickListener mOnItemClickListener;

    public PublicGuideBeanAdapter(Context context, List<PublicGuideBean.ResultsBean> datas) {
        this.datas = datas;
...
//    }
    ...
    public BaseViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == ITEM_NOIMAGE) {
            return new BaseViewHolder(LayoutInflater.from(mContext).inflate(R.layout.item_learder_of_window, parent, false));
    ...
    public void onBindViewHolder(BaseViewHolder holder, final int position) {
        if (!(viewFooter != 0 && position == getItemCount() - 1)) {
            ...
            final PublicGuideBean.ResultsBean item = datas.get(position);
...
//            }
            ...
            holder.setText(R.id.tv_title, datas.get(position).name);
